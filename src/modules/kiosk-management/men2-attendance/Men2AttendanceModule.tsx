...
interface Men2AttendanceModuleProps {
    url?: string;
}

export function Men2AttendanceModule({ url }: Men2AttendanceModuleProps) {
    ...
    const handleRefresh = () => {
        setIsLoading(true);
        const iframe = document.getElementById("men2-attendance-iframe") as HTMLIFrameElement;
        if (iframe) {
    ...
    return (
        <div className="flex flex-col h-screen max-h-[calc(100vh-2rem)] space-y-2 animate-in fade-in duration-700">
            ...
            <div className="flex-1 relative rounded-3xl border border-border/40 shadow-2xl overflow-hidden bg-muted/20 backdrop-blur-xl">
                ...
                <iframe
                    id="men2-attendance-iframe"
                    src={displayUrl}
                    className="w-full h-full border-none"
                    onLoad={() => setIsLoading(false)}
                    title="Men2 Attendance System"
